...
x_axis = scene.addPolygon(QPolygonF([
	QPointF(-5, 0),
	QPointF(5, 0)
]), axis_pen)
x_axis.setZValue(0)
y_axis = scene.addPolygon(QPolygonF([
	QPointF(0, -5),
	QPointF(0, 5)
]), axis_pen)
y_axis.setZValue(0)
...
def update_polygon() -> None:
	...
	unit_square_child_child_child[:] = (unit_square_child_child_child - unit_square_child_child_child[0]) @ np.array([[cos(4 * theta), sin(4 * theta)], [-sin(4 * theta), cos(4 * theta)]]) + unit_square_child_child[2]
